...
public class Parqueadero {
    ...
    public String metodo1() {
        int hactual = darHoraActual();
        int menor = 0;

        for (int i = 0; i < puestos.length; i++) {

            Carro carro = new Carro(darPlacaCarro(i), darHoraActual());
            int  nllegada = carro.darHoraLlegada();

            if (nllegada > menor) {
                return darPlacaCarro(i);
            }

        }
            return String.valueOf(hactual);
        }
